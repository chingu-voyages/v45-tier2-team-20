...
export const ToggleButton = ({ name, onClick }) => {
  ...
  const [isDefaultSortOption, setIsDefaultSortOption] = useState(true);

  const handleClick = () => {
    setIsDefaultSortOption(false);
    setToggle(!toggle);
    if (onClick) {
      onClick();
    }
